...
| Q4 | ~~Per-member permission overrides~~ **DECIDED: deferred.** No per-member overrides for now; the Team screen shows the role's permissions read-only (“From the role”). | Yes (grant/deny single keys) | A role *is* its permission set | `permissions.ts:180` |
