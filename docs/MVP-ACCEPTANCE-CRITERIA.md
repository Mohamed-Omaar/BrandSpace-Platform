...
| **AC-16.8**  | `editVariant` on an `APPROVED` item returns it to `DRAFT` and writes `content.approval_revoked` with `reason: edited_after_approval`. Asserted directly. **Phase 2 closed the other half (D-223):** a `SCHEDULED` item is deliberately NOT unscheduled by an edit — the calendar owns that edge — so the verdict stayed valid over changed words. The approval now records a fingerprint of what it was granted over, the publish preflight recomputes it from the variant it is about to send, and a mismatch is `APPROVAL_REVOKED`. The edit also writes a WARNING `content.scheduled_item_edited` rather than silently rewriting anything |
