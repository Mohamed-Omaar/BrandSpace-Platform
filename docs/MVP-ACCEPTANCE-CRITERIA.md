...
**AC-11.2, AS BUILT AFTER THE PHASE 2 CORRECTION PASS.** "The idempotency key replays the first draft"
now holds for two SIMULTANEOUS submits as well as for a sequential retry. The read-then-create shape
only covered the second: two requests in flight under one key both found no row, both inserted, and the
...
through 17 — the calendar, the approval, the publish job, campaign performance — were all downstream
of a provider decision nobody has taken. **A post can now be written by a person**, through
`ContentLibraryService.createManualItem` and the composer's "Save without AI" control: the same
`ContentItem` and `ContentVariant` rows, the same lifecycle, no model call and **no credit
movement** — `origin: 'HUMAN'` with a null `aiRequestId`, on a service class that holds no AI
gateway. This adds no new criterion above; it makes the criteria below reachable without a vendor.
...
**AC-12.6, AS BUILT AFTER PHASE 2 (D-226).** "Returns the original result" holds for a SUCCESS and
for a failure re-running would only reach again — `MODERATION_BLOCKED` and the deterministic classes
(`CONTENT_FILTERED`, `INVALID_REQUEST`, `CONTEXT_TOO_LONG`). It does NOT hold for a TRANSIENT
failure, and the original reading of it was a defect: a timeout is terminal, so replaying it
permanently poisoned that key — the customer retried, got the recorded failure without anybody being
called, and the request could never succeed. A transient failure now frees the key (the row is kept
and renamed, never deleted) and the retry runs for real. **The charge count is unchanged**, which is
what this criterion is about: a failed request already released its reservation, so there is nothing
outstanding for the retry to duplicate, and exactly one settlement can occur across the pair.
...
| **AC-16.8**  | `editVariant` on an `APPROVED` item returns it to `DRAFT` and writes `content.approval_revoked` with `reason: edited_after_approval`. Asserted directly. **Phase 2 closed the other half (D-223, D-230):** a `SCHEDULED` item is deliberately NOT unscheduled by an edit — the calendar owns that edge — so the verdict stayed valid over changed words. The approval now records a fingerprint of what it was granted over, and the publish preflight recomputes it over the item's COMPLETE current variant set, so an added or removed channel invalidates the verdict as surely as an edited caption does. A mismatch is `APPROVAL_REVOKED`. Title, pillar, tags and campaign are deliberately outside it. An approval predating the column carries NULL, is NOT backfilled, and authorizes nothing. The edit also writes a WARNING `content.scheduled_item_edited` rather than silently rewriting anything |
...
| AC-16.7 | **A connected account gets cursors without anybody creating them** — the sweep ensures them, idempotently and per tenant (D-225) | `phase7-analytics-service`                   |
| AC-16.8 | **A PARTIAL cursor set is repaired too**, and the bounded sweep rotates rather than returning the same head for ever (D-229)     | `phase7-analytics-service`                   |
...
| AC-45.9  | One workspace's failure does not stop the rest of the batch — AND that workspace's own period does not move without its allowance | `phase3-billing-operations` |
| AC-45.10 | A failure DURING the credit reset leaves the period exactly where it was, and the workspace still due                             | `phase3-billing-operations` |
| AC-45.11 | Once the cause is cleared, the same boundary succeeds and grants exactly once                                                     | `phase3-billing-operations` |
| AC-45.12 | Two scheduler instances racing one boundary produce one period advance and one allowance                                          | `phase3-billing-operations` |
| AC-45.13 | A terminal boundary (cancellation, trial expiry) commits its ending and grants zero                                               | `phase3-billing-operations` |
...
| AC-46.7 | A clean record is written only by a COMPLETE rotation that found no drift on any page                        | `reconciliation-rotation`, `phase3-billing-operations` |
| AC-46.8 | A page that finds drift records it at CRITICAL wherever in the rotation it falls                             | `reconciliation-rotation`, `phase3-billing-operations` |
| AC-46.9 | The next rotation starts from a clean slate — one rotation is never described by another's findings          | `reconciliation-rotation`                              |
...
| AC-47.9  | A resource that existed BEFORE its dimension was wired up still fills the ceiling                                                                                                                                                                               | `phase3-connection-quota`, `phase3-total-quota-baseline` |
| AC-47.10 | Two concurrent creates for the last slot, with uncounted history in the way, admit exactly one                                                                                                                                                                  | `phase3-connection-quota`, `phase3-total-quota-baseline` |
| AC-47.11 | Consuming again never double-counts what is already there, and a replayed key consumes nothing                                                                                                                                                                  | `phase3-total-quota-baseline`                            |
| AC-47.12 | Disconnecting a connection the counter never knew about corrupts nothing, and the ceiling still holds                                                                                                                                                           | `phase3-connection-quota`                                |
| AC-47.13 | `limit.storage_gb` is explicitly OUT of the baseline mechanism, with the reason (D-245)                                                                                                                                                                         | `docs/BILLING-AND-CREDITS.md` §30                        |

### AC-49 A subscription that ended stops granting its plan

| ID      | Criterion                                                                                  | Settled by                  |
| ------- | ------------------------------------------------------------------------------------------ | --------------------------- |
| AC-49.1 | Before the period ends, a requested cancellation removes no entitlement                    | `phase3-billing-operations` |
| AC-49.2 | After the terminal boundary, a CANCELLED subscription resolves as a workspace on no plan   | `phase3-billing-operations` |
| AC-49.3 | An EXPIRED trial behaves the same way                                                      | `phase3-billing-operations` |
| AC-49.4 | `Workspace.planKey` is NOT cleared — the commercial record is history, not deleted         | `phase3-billing-operations` |
| AC-49.5 | PAST_DUE keeps full access while dunning runs (§3.5)                                       | `phase3-billing-operations` |
| AC-49.6 | SUSPENDED is explicitly unchanged and still deferred to D-234, asserted so it cannot drift | `phase3-billing-operations` |
| AC-49.7 | A workspace with no subscription at all is unaffected                                      | `phase3-billing-operations` |
