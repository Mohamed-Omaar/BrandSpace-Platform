...
| **Policy per brand**                                       | `requireApprovalBeforeScheduling`, `allowSelfApproval` and `clientApprovalEnabled`, each NULLABLE so an unset brand follows the activated default and a changed default still reaches it                                                                                             |
...
| **D-121 — the Viewer grant (U-06)**                        | `docs/SECURITY.md` §4.3's "optional client approval" is real, per brand, off by default. `client_viewer` still holds `workspace.read` and nothing else                                                                                                                               |
