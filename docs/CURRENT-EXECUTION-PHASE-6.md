...
| 2   | Sidebar **grouping** into CORE / PLAN / CREATE / PUBLISH / IMPROVE / AUTOMATE / WORKSPACE | **NEEDS UX REWORK**    | `AppShell` already takes `readonly ShellNavSection[]` with per-section titles, and `apps/admin` already uses grouped sections (`admin-shell.tsx:54` `NAV_SECTIONS`). The customer dashboard passes **one unnamed section containing all 21 items** (`workspace-shell.tsx:366`). Regrouping uses the shipped mechanism — no new component, no new visual language. |
