...
// A TYPE-ONLY EDGE BACK. `route-scope` imports only `BrandScopeKind` from here
// and types are erased, so the two modules do not form a runtime cycle.
import { scopeForPath } from './route-scope';

/**
 * THE GLOBAL BRAND CONTEXT (D-190, D-191, D-192).
 *
 * WHAT IT REPLACES. Before Phase 8 every brand-scoped screen answered "which
 ...
 *     four brands opened the screen and was editing one of them without ever
 *     having chosen it, and nothing on the page said which.
 ...
 *
 * Four answers to one question, one of them a guess. THE GUESS IS THE DEFECT:
 * a screen that quietly decides which brand you are working on is a screen that
 * can quietly put your work on the wrong one.
 *
 * WHAT THIS IS. One resolution, on the server, for every dashboard request. It
 * lists the brands the MEMBER may act on — filtered in the query by their
 * BrandScope, never read wide and narrowed afterwards (D-132) — and it resolves
 * the selection with a precedence that is written down rather than inferred.
 *
 * THE PRECEDENCE, AND WHY IT IS THIS ONE (D-191):
 *
 ...
 *
 * THE COOKIE IS NOT AN AUTHORIZATION CREDENTIAL, and nothing here treats it as
 * one. It carries `<workspaceId>:<value>`:
 *
 *   - THE WORKSPACE HALF is what makes switching workspace safe. A cookie whose
 *     workspace does not match the live session is ignored OUTRIGHT — we never
 *     ask whether that brand exists in the new workspace, so a switch cannot be
 *     used to probe another tenant's brands. It is not "cleared because the
 *     brand was not found"; it is never looked up at all.
 *   - THE BRAND HALF is re-validated against the member's BrandScope AT QUERY
 *     TIME on every single request. A scope narrowed a minute ago takes effect
 *     on the next page load, because the cookie only ever proposes.
 *
 ...
 * means both shapes pass `workspace` and no page has to be restructured to call
 * this.
 */
 ...
 *
 * `requested` is the route's own `?brand=` value when it reads one, and
 * `undefined` when the route does not take one at all. `null` and `''` are
 * treated as "the URL is silent" so a cleared query string falls back to the
 * stored selection rather than wiping it.
 */
export async function resolveBrandContext(
  ...
  const aggregateAllowed = options.scope === 'brand-or-all';

  const store = await cookies();
  const stored = parseBrandCookie(store.get(BRAND_COOKIE)?.value, source.workspaceId);
 ...
 * branch impossible to forget, because the only way to get an id out of a
...
/**
 * The brand ids a query should be narrowed to, or `undefined` for "no narrowing
 ...
 * an open redirect. The rules are deliberately narrow — narrower than "is it
 * relative?", because `//evil.example` and a backslash-prefixed path are both
 * relative by that test and both leave the site:
 *
