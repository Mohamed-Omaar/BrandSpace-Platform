...
/** Renders the ACTIVE 'ai.routing' configuration for this environment. */
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  await requirePageActor(locale, 'platform.workspace.read');
  const isArabic = locale === 'ar';

  const payload = (await getConfigService().get('ai.routing', currentEnvironment())) as Record<
    string,
    unknown
  >;
  const rows = (payload['rules'] ?? []) as Record<string, unknown>[];
  const columns = [
    'taskKey',
    'scope',
    'primaryModelKey',
  ...
  return (
    <>
      <PageHeading
        title={isArabic ? 'توجيه مهام الذكاء الاصطناعي' : 'AI task routing'}
        description={
          isArabic
