...
/** Create the first workspace through the REAL form. Billing is USD by product policy. */
async function createWorkspace(
  ...
  await page.fill('[data-testid="country-select"]', input.countryLabel);
  await page.getByRole('option', { name: new RegExp(input.countryLabel) }).click();
  ...
  await page.waitForURL(new RegExp(`/${locale}/onboardingimport crypto from 'node:crypto';
import { expect, test, type Page } from '@playwright/test';
import AxeBuilder from '@axe-core/playwright';
import { API_BASE_URL, DASHBOARD_BASE_URL } from './apps';
import { withPlatformPrisma } from './platform-prisma';
...
test.describe('a stranger becomes a paying customer', () => {
  test('signs up, verifies, creates a workspace and reaches a numbered invoice', async ({
    ...
    await createWorkspace(page, { countryLabel: 'Saudi Arabia' });
    ...
    // --- Billing, priced in the product's customer-facing USD currency.
  ...
  test('workspace creation is not blocked by an unconfigured payment market', async ({
    page,
  }) => {
    test.slow();
    ...
    // Great Britain is deliberately absent from the commerce fixture. Country
    // describes the workspace; payment routing is a later checkout concern.
    await createWorkspace(page, { countryLabel: 'United Kingdom' });
...
test.describe('the commercial screens work in both languages', () => {
  test('renders billing right-to-left in Arabic and passes an accessibility scan', async ({
    ...
    await createWorkspace(page, { countryLabel: 'Saudi Arabia' }, 'en');
